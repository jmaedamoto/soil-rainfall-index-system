...
def compare_rain_timelines_corrected(server_timeline, vba_timeline, tolerance=0.01):
    """
    Rain タイムラインの修正版比較（FT時刻オフセット考慮）
    サーバー: ft=3,6,9,12,... / VBA: ft=0,3,6,9,...
    """
    ...
    for i in range(len(server_timeline)):
        ...
        # Rain の FT時刻は3時間ずれている
        expected_server_ft = vba_point['ft'] + 3

        if server_point['ft'] != expected_server_ft:
            return False, f"ft_mismatch: server_ft={server_point['ft']} vs expected={expected_server_ft} (vba_ft={vba_point['ft']}+3)"
...
def compare_swi_timelines(server_timeline, vba_timeline, tolerance=0.01):
    """
    SWI タイムラインの比較（どちらもft=0から開始）
    """
    if len(server_timeline) != len(vba_timeline):
        return False, f"length_diff: server={len(server_timeline)} vs vba={len(vba_timeline)}"
    ...
    for i in range(len(server_timeline)):
        ...
        if server_point['ft'] != vba_point['ft']:
            return False, f"ft_mismatch: server_ft={server_point['ft']} vs vba_ft={vba_point['ft']}"
...
def final_complete_validation():
    """
    修正版比較ロジックによる最終完全一致検証
    """
    print("=== 修正版比較ロジックによる最終完全一致検証 ===")
    print()
            ...
            if server_mesh['area'] == area_name and server_mesh['pref'] == vba_mesh['pref']:
                ...
                if not boundary_match:
                    boundary_mismatches.append({
                        ...
                        'vba_key': vba_key,
                        'area': area_name,
                        'server_boundary': f"{server_mesh['advisary_bound']}/{server_mesh['warning_bound']}/{server_mesh['dosyakei_bound']}",
                        'vba_boundary': f"{vba_mesh.get('advisary_bound', -1)}/{vba_mesh.get('warning_bound', -1)}/{vba_mesh.get('dosyakei_bound', -1)}"
                    })
                    continue
                ...
                rain_match, rain_reason = compare_rain_timelines_corrected(server_rain, vba_rain)
                ...
                swi_match, swi_reason = compare_swi_timelines(server_swi, vba_swi)
    ...
    print(f"   総比較回数: {total_comparisons:,}")
    print(f"   境界値不一致: {len(boundary_mismatches):,}")
    print(f"   Rain不一致: {len(rain_mismatches):,}")
    print(f"   SWI不一致: {len(swi_mismatches):,}")
    ...
    if len(perfect_matches) > 0:
        ...
        if match_rate_server == 100.0 and match_rate_vba == 100.0:
            ...
            if boundary_mismatches:
                print(f"   境界値不一致例: {boundary_mismatches[0]['server_boundary']} vs {boundary_mismatches[0]['vba_boundary']}")
            if rain_mismatches:
                print(f"   Rain不一致例: {rain_mismatches[0]['reason']}")
            if swi_mismatches:
                print(f"   SWI不一致例: {swi_mismatches[0]['reason']}")
    ...
    result_summary = {
        ...
        'rain_mismatches': len(rain_mismatches),
        'swi_mismatches': len(swi_mismatches),
        'match_rate_server': match_rate_server if 'match_rate_server' in locals() else 0,
        'match_rate_vba': match_rate_vba if 'match_rate_vba' in locals() else 0,
        'prefecture_counts': pref_counts if 'pref_counts' in locals() else {},
        'perfect_match_details': perfect_matches[:10] if perfect_matches else []
    }

    with open('final_validation_results.json', 'w', encoding='utf-8') as f:
        json.dump(result_summary, f, ensure_ascii=False, indent=2)

    print(f"   詳細結果を final_validation_results.json に保存しました")

if __name__ == "__main__":
    final_complete_validation()
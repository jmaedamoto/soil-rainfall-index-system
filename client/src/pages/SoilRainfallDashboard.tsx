...
const SoilRainfallDashboard: React.FC = () => {
  ...
  const [data, setData] = useState<CalculationResult | null>(state?.adjustedResult || null);
  ...
  // 時刻入力フィールドの状態（調整結果からの復元またはデフォルト）
  const [swiInitialTime, setSwiInitialTime] = useState(state?.swiInitial || '2023-06-02T00:00:00Z');
  const [guidInitialTime, setGuidInitialTime] = useState(state?.guidanceInitial || '2023-06-02T00:00:00Z');
  const [useSeparateTimes, setUseSeparateTimes] = useState(false);

  // データソース選択の状態（調整結果からの復元またはデフォルト）
  const [dataSource, setDataSource] = useState<'test' | 'production'>(state?.dataSource || 'test');
  const [isAdjustedData, setIsAdjustedData] = useState(!!state?.adjustedResult);
